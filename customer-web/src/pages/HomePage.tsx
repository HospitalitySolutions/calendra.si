...
export function HomePage() {
  ...
  return <div className="page-stack">
    <section className="welcome-row">
      <div><span className="overline">Moj Calendra račun</span><h2>Pozdravljeni, {user?.firstName || 'dobrodošli'}.</h2><p>Vaši termini, ponudniki in ugodnosti na enem mestu.</p></div>
    ...
    {data.nextBooking ? <section className="section-block"><div className="section-heading"><div><span className="overline">Naslednji termin</span><h2>Prihaja kmalu</h2></div><Link to="/termini">Vsi termini <ArrowUpRightIcon size={17}/></Link></div><BookingCard booking={data.nextBooking}/></section>
      : <EmptyState title="Trenutno nimate prihodnjih terminov" description="Poiščite ponudnika in rezervirajte naslednji termin." action={<Link to="/isci" className="button button--primary">Razišči ponudnike</Link>} />}
    ...
    {data.activeEntitlements.length > 0 && <section className="section-block"><div className="section-heading"><div><span className="overline">Vaša denarnica</span><h2>Aktivne ugodnosti</h2></div><Link to="/denarnica">Odpri denarnico <ArrowUpRightIcon size={17}/></Link></div><div className="card-grid card-grid--3">{data.activeEntitlements.slice(0, 3).map(item => <EntitlementCard key={item.entitlement.entitlementId} item={item}/>)}</div></section>}
