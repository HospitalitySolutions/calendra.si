...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$PSScriptRoot\backend'; `$env:SPRING_PROFILES_ACTIVE='local'; `$env:SPRING_DATASOURCE_URL='jdbc:postgresql://localhost:5432/calendradb'; `$env:SPRING_DATASOURCE_USERNAME='calendra'; `$env:SPRING_DATASOURCE_PASSWORD='calendra'; `$env:SPRING_FLYWAY_ENABLED='true'; `$env:SPRING_FLYWAY_BASELINE_ON_MIGRATE='false'; `$env:SPRING_JPA_HIBERNATE_DDL_AUTO='validate'; `$env:APP_JWT_SECRET='local-development-jwt-secret-change-me'; `$env:APP_SETTINGS_ENCRYPTION_KEY='local-development-settings-key-change-me'; mvn spring-boot:run"
