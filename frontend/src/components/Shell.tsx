...
function ShellInner({ children, user: authenticatedUser }: ShellProps) {
  ...
  const triggerGlobalVoice = () => {
    ...
    if (!isCalendarRoute) {
      navigate('/calendar')
      // Fire twice to cover route transition timing (calendar may mount after first tick).
  ...
  const userAvatarSrc = String(user.avatarPath || '').trim()
  ...
  const onProfileAvatarPicked = async (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0]
    event.target.value = ''
    if (!file) return
    if (file.size > 5 * 1024 * 1024) {
      window.alert(locale === 'sl' ? 'Slika je lahko velika največ 5 MB.' : 'Avatar image must be 5 MB or smaller.')
      return
    }
    if (!(file.type || '').toLowerCase().startsWith('image/')) {
      window.alert(locale === 'sl' ? 'Dovoljene so samo slikovne datoteke.' : 'Only image files are allowed.')
      return
    }
    ...
    try {
      ...
      const { data } = await api.post('/users/profile/avatar', payload, { headers: { 'Content-Type': 'multipart/form-data' } })
      const previousRaw = sessionStorage.getItem('user')
      ...
      const backendMessage = typeof error?.response?.data?.message === 'string'
        ...
        : ''
      window.alert(backendMessage || (locale === 'sl' ? 'Nalaganje slike ni uspelo.' : 'Failed to upload avatar image.'))
    } finally {
    ...
    createPortal(
      <div className="mobile-nav-overlay" role="dialog" aria-modal="true" aria-label={t('mobileNavMenu')}>
        <header className="mobile-nav-overlay-header">
          <span className="mobile-nav-overlay-brand">{headerBrandLabel}</span>
          <button
            type="button"
        ...
        <div className="mobile-nav-overlay-body">
          ...
          <div className="mobile-nav-overlay-body-settings">
            ...
            {canScanWalletEntitlements && (
              <NavLink
                to="/scanner"
                className={({ isActive }) => `mobile-nav-overlay-link${isActive ? ' active' : ''}`}
                onClick={() => closeMobileNavIfAlreadyOn('/scanner')}
              >
                <span className="mobile-nav-overlay-link-icon">
                  <SidebarIconScanner />
                </span>
                <span className="mobile-nav-overlay-link-label">{t('navWalletScanner')}</span>
              </NavLink>
            )}
  ...
  const calendarVoiceInHeader = calendarShellSlots?.voiceButton ?? null
  const avatarUploadInput = (
    <input
      ...
      accept="image/*"
      style={{ display: 'none' }}
      onChange={onProfileAvatarPicked}
    />
  )
  ...
  return (
    <div className="layout">
          ...
          {(canViewServices || canViewEmployees || canScanWalletEntitlements) && (
            <div className="sidebar-rail-footer">
              ...
              {canScanWalletEntitlements && (
                <NavLink
                  className={({ isActive }) =>
                    `sidebar-scanner sidebar-rail-link${isActive ? ' active' : ''}`
                  }
                  to="/scanner"
                  title={t('navWalletScanner')}
                  aria-label={t('navWalletScanner')}
                >
                  <span className="sidebar-rail-link-icon" aria-hidden>
                    <SidebarIconScanner />
                  </span>
                  <span className="sidebar-rail-link-label">{t('navWalletScanner')}</span>
                </NavLink>
              )}
            </div>
          )}
      ...
      <div
        ref={mainAreaRef}
        className={isCalendarRoute ? 'main-area main-area--calendar' : isClientsRoute ? 'main-area main-area--clients' : isWaitlistRoute ? 'main-area main-area--waitlist' : isServicesRoute ? 'main-area main-area--services' : isEmployeesRoute ? 'main-area main-area--employees' : isAnalyticsRoute ? 'main-area main-area--analytics' : isBillingRoute ? 'main-area main-area--billing' : isAccountManagementRoute ? 'main-area main-area--configuration-account' : isNotificationsConfigurationRoute ? 'main-area main-area--configuration-notifications' : isWhiteConfigurationDetailRoute ? 'main-area main-area--configuration-detail-white' : 'main-area'}
      >
        <header
          ref={headerRef}
          className={
        ...
        <main className={isCalendarRoute ? 'content content--calendar-flush' : isClientsRoute ? 'content content--clients' : isWaitlistRoute ? 'content content--waitlist' : isServicesRoute ? 'content content--services' : isEmployeesRoute ? 'content content--employees' : isAnalyticsRoute ? 'content content--analytics' : isBillingRoute ? 'content content--billing' : isAccountManagementRoute ? 'content content--configuration-account' : isNotificationsConfigurationRoute ? 'content content--configuration-notifications' : isWhiteConfigurationDetailRoute ? 'content content--configuration-detail-white' : 'content'}>{children}</main>
      </div>
      {mobileNavOverlay}
      {globalVoiceButton}
