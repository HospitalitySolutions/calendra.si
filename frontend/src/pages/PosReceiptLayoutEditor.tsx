...
export function PosReceiptLayoutEditor({ hidePreview = false, onSaved }: { hidePreview?: boolean; onSaved?: () => void } = {}) {
  ...
  const copy = useMemo(() => {
    if (locale === 'sl') {
      return {
        ...
        showPaymentTypeHint: 'V razdelku Načini plačila prikaže uporabljene vrste plačila (npr. Gotovina in Kartica).',
    ...
    if (locale === 'sr') {
      return {
        ...
        showPaymentTypeHint: 'U odeljku Načini plaćanja prikazuje korišćene vrste plaćanja (npr. Gotovina i Kartica).',
        save: 'Sačuvaj izgled',
    ...
    return {
      ...
      showPaymentTypeHint: 'Shows used payment types in the Payment methods section (for example Cash and Card).',
      save: 'Save layout',
      reset: 'Reset',
      test: 'Test print',
  ...
  const visibleSection = (section: string) => {
    ...
    // The VAT section also owns the discount/subtotal prelude, so it remains
    // in the receipt even when the VAT-breakdown toggle itself is off.
    if (section === 'vat') return true
  ...
  const previewSections: Record<string, ReactNode> = {
    ...
    document: <><hr /><strong className="pos58-preview-title">{locale === 'en' ? 'Invoice:' : 'Račun:'} MB-1-2026-00042</strong><div className="pos58-preview-document-gap" /><div><span>{locale === 'sl' ? 'Izdano' : locale === 'sr' ? 'Izdato' : 'Issued'}</span><b>Maribor, 31.07.2026 12:45</b></div><div><span>{locale === 'sl' ? 'Datum opr. storitve' : locale === 'sr' ? 'Datum izvršene usluge' : 'Service date'}</span><b>31.07.2026</b></div><div><span>{locale === 'sl' ? 'Rok plačila' : locale === 'sr' ? 'Rok plaćanja' : 'Due date'}</span><b>07.08.2026</b></div><hr /></>,
    ...
    advancePayments: <></>,
    totals: <><div className="pos58-preview-summary-row pos58-preview-summary-row--strong"><span>{locale === 'sl' ? 'Skupaj EUR' : locale === 'sr' ? 'Ukupno EUR' : 'Total EUR'}</span><b>28,00</b></div><div className="pos58-preview-summary-row"><span>{locale === 'sl' ? 'Plačano EUR' : locale === 'sr' ? 'Plaćeno EUR' : 'Paid EUR'}</span><b>28,00</b></div><hr className="pos58-preview-summary-divider" /><div className="pos58-preview-summary-row pos58-preview-summary-row--strong"><span>{locale === 'sl' ? 'Za plačilo EUR' : locale === 'sr' ? 'Za plaćanje EUR' : 'Amount due EUR'}</span><b>0,00</b></div>{layout.showPaymentDetails ? <><div className="pos58-preview-summary-spacer" /><strong>{locale === 'sl' ? 'Načini plačila' : locale === 'sr' ? 'Načini plaćanja' : 'Payment methods'}</strong><div className="pos58-preview-summary-row"><span>{locale === 'sl' ? 'Gotovina' : locale === 'sr' ? 'Gotovina' : 'Cash'}</span><b>28,00</b></div></> : null}<hr className="pos58-preview-summary-rule" /></>,
    vat: <><div className="pos58-preview-summary-row"><span>{locale === 'sl' || locale === 'sr' ? 'Popust' : 'Discount'}</span><b>- 2,00</b></div><div className="pos58-preview-summary-row"><span>{locale === 'sl' ? 'Skupaj brez DDV' : locale === 'sr' ? 'Ukupno bez PDV-a' : 'Total excl. VAT'}</span><b>30,00</b></div>{layout.showVatBreakdown ? <div className="pos58-preview-summary-row pos58-preview-vat-row"><span>22% · {locale === 'sl' ? 'osnova' : locale === 'sr' ? 'osnovica' : 'basis'} 22,95</span><b>5,05</b></div> : null}</>,
