...
function getStoredRegisterSelectionSearch() {
  ...
  try {
    const stored = window.sessionStorage.getItem(
      REGISTER_SELECTION_STORAGE_KEY,
    );
...
function storePendingBillingDetailsRedirect(search?: string | null) {
  ...
  try {
    ...
    window.sessionStorage.setItem(
      REGISTER_BILLING_DETAILS_SEARCH_KEY,
      normalized,
    );
...
const accountPageCopy: Record<"en" | "sl", AccountPageCopy> = {
  en: {
    ...
    passwordHint:
      "The page now uses the same register shell, with the account setup card centered.",
    ...
    verifyHelp:
      "Enter the newest code from your inbox. If needed, resend a fresh code or use another email.",
    finishVerifyTitle: "Finish verifying your account",
    finishVerifySubtitle:
      "This email already started account setup but is waiting for verification. Enter the newest code to continue.",
    finishVerifyNextStep: "Continue your setup",
    finishVerifyHelp:
      "We can resend a fresh code, or you can switch to another email.",
    ...
    registeredSubtitle:
      "This email already belongs to a verified account. Sign in to continue where you left off.",
    ...
    invalidSubtitle:
      "The verification challenge may have expired or already been used. We can send you a fresh code.",
  ...
  sl: {
    ...
    passwordHint:
      "Stran zdaj uporablja isto register ogrodje, kartica za nastavitev računa pa je na sredini.",
    ...
    verifyHelp:
      "Vnesite najnovejšo kodo iz e-pošte. Po potrebi pošljite novo kodo ali uporabite drug e-mail.",
    finishVerifyTitle: "Dokončajte potrditev računa",
    finishVerifySubtitle:
      "Ta e-mail je že začel nastavitev računa, vendar še čaka potrditev. Vnesite najnovejšo kodo.",
    ...
    registeredSubtitle:
      "Ta e-mail pripada že potrjenemu računu. Prijavite se in nadaljujte.",
    ...
    invalidSubtitle:
      "Verifikacijski izziv je morda potekel ali že uporabljen. Lahko pošljemo novo kodo.",
...
export function RegisterAccountPage() {
  ...
  >("form");
  const [verifyScenario, setVerifyScenario] = useState<"new" | "existing">(
    "new",
  );
  ...
  const peekAddonMonthly = useMemo(() => {
    if (selection.billing === "annual") {
      return (
        (monthlyAmounts.usersMonthly + monthlyAmounts.addonsMonthly) *
          getAnnualDiscountFactor() +
  ...
  const persistSignupSession = async (
    ...
    const addr = (emailOverride !== undefined ? emailOverride : email)
      .trim()
      .toLowerCase();
  ...
  const navigateToExistingAccount = (
    addr: string,
    payload?: SignupSelectionPayload | null,
  ) => {
    const q = queryWithRegisterSelection(location.search, payload);
  ...
  const submitSignup = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (check?.pendingVerification) {
        ...
        const { data: resend } = await api.post<SignupResendCodeResponse>(
      ...
      if (axios.isAxiosError(err) && err.response?.status === 409) {
        ...
        if (d?.pendingVerification && d?.email) {
          ...
          try {
            ...
            openFinishVerify(
              String(resend?.email || pendingEmail),
              String(resend?.challengeId || ""),
              resend,
            );
          } catch {
  ...
  const resendVerificationEmail = async () => {
    ...
    try {
      ...
      const { data } = await api.post<SignupResendCodeResponse>(
        "/auth/signup/resend-code",
        {
      ...
      const nextChallenge = String(
        data?.challengeId || verifyChallengeId || "",
      );
      setVerifyEmailAddr(nextEmail);
      setVerifyChallengeId(nextChallenge);
      setVerificationCode("");
  ...
  const submitVerification = async () => {
    ...
    if (
      ...
      !/\d/.test(verifyPassword) ||
      !/[A-Z]/.test(verifyPassword) ||
      !/[a-z]/.test(verifyPassword)
    ) {
      setError(
    ...
    setError("");
    try {
      const { data } = await api.post<SignupVerifyCodeResponse>(
      ...
      if (data?.user) {
        ...
        const nextSearch = resolveRegisterSelectionSearch(
          data,
          location.search,
          selection,
        );
        ...
        const requiresBillingDetails =
          selectionRequiresBillingDetails(nextSelection);
      ...
      if (axios.isAxiosError(err)) {
        ...
        setError(message);
        if (
          err.response?.data?.invalidVerificationCode ||
          err.response?.data?.invalidVerificationLink
        ) {
          openInvalidVerify(
            verifyEmailAddr,
            verifyChallengeId,
            err.response?.data as SignupSelectionPayload,
          );
        }
      } else {
        setError(t("signupFailed"));
  ...
  const startGoogleSignup = async () => {
    ...
    try {
      await persistSignupSession(optionalWork, {
  ...
  const useAnotherEmail = () => {
    ...
    navigate(next ? `/register/account?${next}` : "/register/account", {
      replace: true,
    });
    setView("form");
    setVerifyScenario("new");
                ...
                {view === "invalid" ? (
                  <>
                    <div className="register-account-verify-badges">
                      <span className="register-account-verify-badge register-account-verify-badge--blue">
                        {copy.verifyBadgeAccount}
                      </span>
                      <span className="register-account-verify-badge register-account-verify-badge--gold">
                        {copy.invalidBadge}
                      </span>
                    </div>
                    ...
                    <div className="register-account-verify-card">
                      ...
                      <button
                        type="button"
                        className="register-account-verify-link"
                  ...
                  <>
                    ...
                    <div className="register-account-header">
                      <div className="register-account-title-row">
                        <div className="register-account-title-block">
                          <h1 className="register-account-title">
                            {copy.registeredTitle}
                          </h1>
                          <p className="register-account-copy">
                            {copy.registeredSubtitle}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="register-account-verify-card">
                      ...
                      <button
                        ...
                        onClick={() =>
                          navigate(
                            `/login?email=${encodeURIComponent(verifyEmailAddr)}`,
                          )
                        }
                      >
                      ...
                      >
                        {registeredResetSending
                          ? copy.verifyResending
                  ...
                  <>
                    <div className="register-account-verify-badges">
                      ...
                      {verifyScenario === "existing" ? (
                        <span className="register-account-verify-badge register-account-verify-badge--gold">
                          {copy.verifyBadgeExisting}
                        </span>
                      ) : (
                        <span className="register-account-verify-badge register-account-verify-badge--green">
                          {copy.verifyBadgeSent}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="register-account-verify-card">
                      <p className="register-account-verify-next">
                        {verifyScenario === "existing"
                          ? copy.finishVerifyNextStep
                          : copy.verifyNextStep}
                      </p>
                      <p className="register-account-verify-hint">
                        {verifyScenario === "existing"
                          ? copy.finishVerifyHelp
                          : copy.verifyHelp}
                      </p>
                      <div className="register-account-verify-email-pill">
                        <span aria-hidden>✉</span>
                      ...
                      <div className="register-account-field">
                        <label htmlFor="register-account-verify-code">
                      ...
                      <div className="register-account-field">
                        <label htmlFor="register-account-verify-password">
                          {copy.verifyPasswordLabel}
                        </label>
                        <input
                      ...
                      <div className="register-account-field">
                        <label htmlFor="register-account-verify-password-repeat">
                          {copy.verifyConfirmPasswordLabel}
                        </label>
                        <input
                          ...
                          onChange={(e) =>
                            setVerifyConfirmPassword(e.target.value)
                          }
                      ...
                      <div className="register-account-verify-divider">
                        {copy.divider}
                      </div>
                      <button
                        type="button"
                  ...
                  <>
                    ...
                    <div className="register-account-form-card">
                      <form
                        className="register-account-form"
                        onSubmit={submitSignup}
                      >
                        <div className="register-account-field">
                          <label htmlFor="register-account-email">
                            {copy.workEmailLabel}
                          </label>
                          <input
                        ...
                        <div className="register-account-divider">
                          {copy.divider}
                        </div>

                        <button
                          className="register-account-social-button"
                          type="button"
                          onClick={() => void startGoogleSignup()}
                        >
                          <span
                            className="register-account-google-mark"
                            aria-hidden
                          >
                            <svg
                              width="16"
                        ...
                        <div className="register-account-helper-row">
                          <div className="register-account-helper-text">
                            {copy.helperText}
                          </div>
                          <button
                            className="register-account-link-button"
                            type="button"
                        ...
                        <button
                          ...
                          onClick={() =>
                            navigate(
                              `/register?${selectionToSearch(selection)}`,
                            )
                          }
                        >
                          {copy.back}
                ...
                <button
                  ...
                  aria-label={
                    footerExpanded
                      ...
                      : registerShell.footerShowDetails
                  }
          ...
          {footerExpanded ? (
            ...
            >
              <div className="register-footer-peek">
                ...
                <div className="register-footer-peek-col">
                  <span className="register-footer-peek-label">
                    {registerShell.footerUsagePeek}
                  </span>
                  <strong className="register-footer-peek-name">
                    {usageAddonLineCount}{" "}
                    {usageAddonLineCount === 1
                      ? registerShell.footerItemSingular
                      : registerShell.footerItemPlural}
                  </strong>
                  <span className="register-footer-peek-value">
              ...
              <div className="register-footer-detail-card">
                <h3 className="register-footer-detail-title">
                  {registerShell.footerBreakdownTitle}
                </h3>
      ...
      {contactOpen ? (
        <div className="register-contact-modal-root" role="presentation">
          <button
            ...
            aria-label={registerShell.contactCloseBackdrop}
          ...
          >
            ...
            <p className="register-contact-modal-intro">
              {registerShell.contactIntro}
            </p>
            <div className="register-contact-form stack gap-md">
              ...
              <Field
                label={registerShell.contactPhone}
                hint={registerShell.contactPhoneHint}
              >
                <input
                  value={contactPhone}
              ...
              <Field label={registerShell.contactMessage}>
                <textarea
                  ...
                  placeholder={registerShell.contactPlaceholder}
                />
              </Field>
              {contactError ? (
