          ...
          {loadingPurchaseOrder ? <PanelEmpty>Nalagam naročilnico…</PanelEmpty> : <>
            ...
            >
              <div className="consumables-modal-grid procurement-header-grid">
                ...
                <label>Poslovalnica<DesktopSelect value={purchaseOrderForm.locationId} disabled={!canManageProcurement || purchaseOrderTerminal || purchaseOrderHasReceipts} onChange={(e) => setPurchaseOrderForm((f) => ({ ...f, locationId: e.target.value, lines: [] }))}><option value="">Izberite</option>{writableInventoryLocations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</DesktopSelect></label>
                <label>Status<DesktopSelect value={purchaseOrderForm.status} disabled={!canManageProcurement || purchaseOrderTerminal || purchaseOrderHasReceipts} onChange={(e) => setPurchaseOrderForm((f) => ({ ...f, status: e.target.value as PurchaseOrderFormState['status'] }))}><option value="DRAFT">Osnutek</option><option value="ORDERED">Naročeno</option>{!purchaseOrderHasReceipts && <option value="CANCELLED">Preklicano</option>}{purchaseOrderForm.status === 'PARTIALLY_RECEIVED' && <option value="PARTIALLY_RECEIVED">Delno prejeto</option>}{purchaseOrderForm.status === 'COMPLETED' && <option value="COMPLETED">Zaključeno</option>}</DesktopSelect></label>
...
function Filters({ query, setQuery, categoryFilter, setCategoryFilter, locationFilter, setLocationFilter, categories, locations, extra }: {
  ...
  return (
    <div className="consumables-filter-row">
      ...
      <label>Kategorija<DesktopSelect value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)}><option value="">Vse kategorije</option>{categories.map((c) => <option key={c.id} value={c.id}>{c.name}{c.active ? '' : ' (neaktivna)'}</option>)}</DesktopSelect></label>
      <label>Lokacija<DesktopSelect value={locationFilter} onChange={(e) => setLocationFilter(e.target.value)}><option value="">Vse lokacije</option>{locations.map((l) => <option key={l} value={l}>{l}</option>)}</DesktopSelect></label>
      {extra}
    </div>
  )
}
...
function ProcurementTab({ orders, items, suppliers, createPurchaseOrder, openPurchaseOrder, createSuggestedOrder, highlightItemId, canManage }: {
  ...
  return <div className="consumables-main-with-side">
    <div>
      ...
      <div className="consumables-filter-row"><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Išči po naročilnici, dobavitelju ali poslovalnici…" /><label>Dobavitelj<DesktopSelect value={supplierFilter} onChange={(e) => setSupplierFilter(e.target.value)}><option value="">Vsi dobavitelji</option>{suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</DesktopSelect></label><label>Status<DesktopSelect value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}><option value="">Vsi statusi</option><option value="DRAFT">Osnutek</option><option value="ORDERED">Naročeno</option><option value="PARTIALLY_RECEIVED">Delno prejeto</option><option value="COMPLETED">Zaključeno</option><option value="CANCELLED">Preklicano</option></DesktopSelect></label><button type="button" className="btn secondary" onClick={reset}>Ponastavi filtre</button></div>
...
function SuppliersTab({ suppliers, createSupplier, openSupplier, canManage }: { suppliers: Supplier[]; createSupplier: () => void; openSupplier: (supplier: Supplier) => void; canManage: boolean }) {
  ...
  return <>
    <div className="consumables-filter-row"><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Išči dobavitelje, kontaktne osebe, e-mail, kategorije…" /><label>Status<DesktopSelect value={status} onChange={(e) => setStatus(e.target.value)}><option value="">Vsi statusi</option><option value="ACTIVE">Aktivni</option><option value="INACTIVE">Neaktivni</option></DesktopSelect></label><label>Kategorija<DesktopSelect value={category} onChange={(e) => setCategory(e.target.value)}><option value="">Vse kategorije</option>{categoryOptions.map((value) => <option key={value} value={value}>{value}</option>)}</DesktopSelect></label><label>Pogoji plačila<DesktopSelect value={paymentTerms} onChange={(e) => setPaymentTerms(e.target.value)}><option value="">Vsi pogoji</option><option value="short">Do 14 dni</option><option value="standard">15–30 dni</option><option value="long">Nad 30 dni</option></DesktopSelect></label><button type="button" className="btn secondary" onClick={reset}>Ponastavi filtre</button></div>
  ...
  return <div className="consumables-main-with-side">
    <div>
      ...
      <div className="consumables-filter-row inventory-filters">
        <label>Inventura<DesktopSelect value={session.id} onChange={(e) => onOpenSession(Number(e.target.value))}>{sessions.map((row) => <option key={row.id} value={row.id}>{row.locationName} · {date(row.startedAt)} · {row.status === 'IN_PROGRESS' ? 'V teku' : 'Zaključena'}</option>)}</DesktopSelect></label>
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Išči po artiklu ali kategoriji…" />
        <label>Kategorija<DesktopSelect value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)}><option value="">Vse kategorije</option>{categories.map((category) => <option key={category} value={category}>{category}</option>)}</DesktopSelect></label>
        <label>Status štetja<DesktopSelect value={countStatusFilter} onChange={(e) => setCountStatusFilter(e.target.value)}><option value="">Vsi statusi</option><option value="UNCOUNTED">Ni prešteto</option><option value="COUNTED">Prešteto</option><option value="DISCREPANCY">Odstopanje</option><option value="MATCH">Ujema se</option></DesktopSelect></label>
...
function ReportsTab({ report, loading, reportType, setReportType, from, setFrom, to, setTo, locationId, setLocationId, serviceTypeId, setServiceTypeId, employeeId, setEmployeeId, locations, onRefresh, onExportCsv, onExportExcel }: {
  ...
  return <div className="consumables-reports">
    <div className="consumables-report-toolbar">
      ...
      <label>Poslovalnica<DesktopSelect value={locationId} onChange={(e) => setLocationId(e.target.value)}><option value="">Vse poslovalnice</option>{locations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</DesktopSelect></label>
      {reportType === 'CONSUMPTION' && <><label>Storitev<DesktopSelect value={serviceTypeId} onChange={(e) => setServiceTypeId(e.target.value)}><option value="">Vse storitve</option>{(report?.serviceOptions || []).map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DesktopSelect></label><label>Izvajalec<DesktopSelect value={employeeId} onChange={(e) => setEmployeeId(e.target.value)}><option value="">Vsi izvajalci</option>{(report?.employeeOptions || []).map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DesktopSelect></label></>}
