...
export function AppointmentsPage() {
  ...
  return <div className="appointments-page">
    ...
    {showFilters && <div className="waitlist-modal-backdrop" onMouseDown={() => setShowFilters(false)}><div className="waitlist-modal compact waitlist-filter-modal" onMouseDown={event => event.stopPropagation()}><header><h2>{copy.filters}</h2><button type="button" className="icon-button" onClick={() => setShowFilters(false)} aria-label={copy.close}>{icon('close')}</button></header><div className="waitlist-form-grid">
    ...
    </div><footer className="waitlist-filter-modal__footer"><button type="button" onClick={resetFilterDraft}>{copy.resetFilters}</button><button type="button" className="primary" onClick={applyFilters}>{copy.applyFilters}</button></footer></div></div>}
