...
function sourceLabel(source: string, locale: string) {
  const sl: Record<string, string> = { WIDGET: 'Spletni vtičnik', PUBLIC_BOOKING_PAGE: 'Javna rezervacijska stran', GUEST_APP: 'Aplikacija za goste', STAFF: 'Osebje', PHONE: 'Telefon', OTHER: 'Drugo' }
  const en: Record<string, string> = { WIDGET: 'Website widget', PUBLIC_BOOKING_PAGE: 'Public booking page', GUEST_APP: 'Guest app', STAFF: 'Staff', PHONE: 'Phone', OTHER: 'Other' }
...
export function AppointmentsPage() {
  ...
  return <div className="appointments-page">
    ...
    {showFilters && <div className="waitlist-modal-backdrop" onMouseDown={() => setShowFilters(false)}><div className="waitlist-modal compact waitlist-filter-modal" onMouseDown={event => event.stopPropagation()}><header className="waitlist-filter-modal__header"><h2>{copy.filters}</h2><button type="button" className="waitlist-filter-modal__close" onClick={() => setShowFilters(false)} aria-label={copy.close}>{icon('close')}</button></header><div className="waitlist-form-grid waitlist-filter-modal__body">
      ...
      <label><span>{copy.source}</span><select value={filterDraft.sourceFilter} onChange={event => setFilterDraft(value => ({ ...value, sourceFilter: event.target.value }))}><option value="">{copy.allSources}</option><option value="WIDGET">{sourceLabel('WIDGET', locale)}</option><option value="PUBLIC_BOOKING_PAGE">{sourceLabel('PUBLIC_BOOKING_PAGE', locale)}</option><option value="GUEST_APP">{sourceLabel('GUEST_APP', locale)}</option><option value="STAFF">{sourceLabel('STAFF', locale)}</option><option value="PHONE">{sourceLabel('PHONE', locale)}</option></select></label>
    ...
    <style>{`
      ...
      @media(max-width:720px){
        ...
        .waitlist-view-tabs{position:sticky;top:calc(61px + env(safe-area-inset-top));z-index:190;display:flex;align-items:stretch;width:100%;min-height:58px;margin:0;padding:0 14px;gap:0;border:0;border-radius:0;background:linear-gradient(135deg,#0b71ee 0%,#0865db 100%);box-shadow:none;box-sizing:border-box;overflow:visible}
