...
const waitlistClientInitials = (name: string) => name.split(/\s+/).slice(0, 2).map(value => value[0]).join('').toUpperCase()
...
export function AppointmentsPage() {
  ...
  const copy = locale === 'sl' ? {
    ...
    active: 'Aktivno', offered: 'Ponudba', history: 'Zgodovina', add: 'Dodaj na čakalno vrsto', search: 'Išči po imenu, e-pošti ali telefonu', filters: 'Filtri',
    ...
    offerExpires: 'Ponudba poteče čez', temporaryHold: 'Termin je začasno rezerviran za stranko do', close: 'Zapri', save: 'Shrani', cancel: 'Prekliči', resetFilters: 'Ponastavi', applyFilters: 'Uporabi filtre',
    ...
    active: 'Active', offered: 'Offer', history: 'History', add: 'Add to waitlist', search: 'Search by name, email or phone', filters: 'Filters',
    ...
    offerExpires: 'Offer expires in', temporaryHold: 'The slot is temporarily held for the client until', close: 'Close', save: 'Save', cancel: 'Cancel', resetFilters: 'Reset', applyFilters: 'Apply filters',
  ...
  const openOffer = (request: WaitlistRequest | null = selected) => {
    ...
    const employeeId = request.specificEmployee?.id
      ? String(request.specificEmployee.id)
  ...
  useEffect(() => {
    setMobilePage(1)
  }, [view, search, serviceFilter, employeeFilter, sourceFilter, dateFromFilter, dateToFilter])

  const mobilePageSize = 10
  const mobilePagination = useMemo(() => {
    const total = rows.length
    const totalPages = Math.max(1, Math.ceil(total / mobilePageSize))
    const page = Math.min(mobilePage, totalPages)
    const start = total === 0 ? 0 : (page - 1) * mobilePageSize
    ...
    return {
      ...
      totalPages,
      slice,
      showFrom: total === 0 ? 0 : start + 1,
      showTo: total === 0 ? 0 : start + slice.length,
    }
  }, [mobilePage, rows])
  ...
  const openFilters = () => {
    setFilterDraft({ serviceFilter, employeeFilter, sourceFilter, dateFromFilter, dateToFilter })
    setShowFilters(true)
  }

  const applyFilters = () => {
    setServiceFilter(filterDraft.serviceFilter)
    setEmployeeFilter(filterDraft.employeeFilter)
    setSourceFilter(filterDraft.sourceFilter)
    setDateFromFilter(filterDraft.dateFromFilter)
    setDateToFilter(filterDraft.dateToFilter)
  ...
  return <div className="appointments-page">
      <div className="waitlist-view-tabs" role="tablist" aria-label={locale === 'sl' ? 'Pogledi čakalne vrste' : 'Waitlist views'}>
        {(['ACTIVE', 'OFFERED', 'HISTORY'] as const).map(item => <button key={item} type="button" role="tab" aria-selected={view === item} className={view === item ? 'active' : ''} onClick={() => { setView(item); void closeSelected({ refreshRows: false, suppressNextReload: false }) }}>
          ...
          <strong className="waitlist-tab-count">{viewCounts[item]}</strong>
        </button>)}
      </div>
      <section className="waitlist-toolbar">
        ...
        <div className="waitlist-toolbar__actions">
          <button type="button" className="waitlist-filter-trigger" onClick={openFilters}>
            {icon('filter')}
        ...
        <section className="waitlist-table-card">
          {isMobileWaitlist ? (
            <div className="waitlist-mobile">
              ...
              {!loading && rows.length > 0 && mobilePagination.totalPages > 1 ? (
                <div className="waitlist-mobile__pagination">
                  <button type="button" className="secondary" onClick={() => setMobilePage(page => Math.max(1, page - 1))} disabled={mobilePagination.page <= 1} aria-label={locale === 'sl' ? 'Prejšnja stran' : 'Previous page'}>‹</button>
                  <span>{mobilePagination.page}</span>
                  <button type="button" className="secondary" onClick={() => setMobilePage(page => Math.min(mobilePagination.totalPages, page + 1))} disabled={mobilePagination.page >= mobilePagination.totalPages} aria-label={locale === 'sl' ? 'Naslednja stran' : 'Next page'}>›</button>
                </div>
              ) : null}
            </div>
          ) : (
              ...
              <table>
                ...
                <tbody>
                  {loading ? <tr><td colSpan={7} className="waitlist-empty">…</td></tr> : rows.length === 0 ? <tr><td colSpan={7} className="waitlist-empty">{copy.noRows}</td></tr> : rows.map(row => <tr key={row.id} className={selected?.id === row.id ? 'selected' : ''} onClick={() => void selectRequest(row)}>
                    <td><div className="waitlist-client"><span className="waitlist-avatar">{waitlistClientInitials(row.clientName)}</span><div><strong>{row.clientName}</strong><small>{row.clientPhone || row.clientEmail || '—'}</small></div></div></td>
    ...
    {showFilters && <div className="waitlist-modal-backdrop" onMouseDown={() => setShowFilters(false)}><div className="waitlist-modal compact waitlist-filter-modal" onMouseDown={event => event.stopPropagation()}><header className="waitlist-filter-modal__header"><h2>{copy.filters}</h2><button type="button" className="waitlist-filter-modal__close" onClick={() => setShowFilters(false)} aria-label={copy.close}>{icon('close')}</button></header><div className="waitlist-form-grid waitlist-filter-modal__body">
      <label><span>{copy.service}</span><select value={filterDraft.serviceFilter} onChange={event => setFilterDraft(value => ({ ...value, serviceFilter: event.target.value }))}><option value="">{copy.allServices}</option>{serviceOptions.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
      <label><span>{copy.employee}</span><select value={filterDraft.employeeFilter} onChange={event => setFilterDraft(value => ({ ...value, employeeFilter: event.target.value }))}><option value="">{copy.allEmployees}</option>{employeeOptions.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
      <label><span>{copy.source}</span><select value={filterDraft.sourceFilter} onChange={event => setFilterDraft(value => ({ ...value, sourceFilter: event.target.value }))}><option value="">{copy.allSources}</option><option value="WIDGET">{sourceLabel('WIDGET', locale)}</option><option value="GUEST_APP">{sourceLabel('GUEST_APP', locale)}</option><option value="STAFF">{sourceLabel('STAFF', locale)}</option><option value="PHONE">{sourceLabel('PHONE', locale)}</option></select></label>
    ...
    <style>{`
      ...
      /* Keep the mobile waitlist controls visually identical to the billing controls. */
      @media(max-width:760px){
        .waitlist-view-tabs{display:flex;align-items:center;width:100%;gap:8px;margin:0 0 14px;padding:2px 10px 6px 0;border:0;border-bottom:1px solid #e6edf5;border-radius:0;background:transparent;overflow-x:auto;overflow-y:hidden;scroll-snap-type:x proximity;scroll-padding-inline:12px;scrollbar-width:none;-ms-overflow-style:none;-webkit-overflow-scrolling:touch;mask-image:linear-gradient(to right,#000 0,#000 calc(100% - 24px),transparent 100%);-webkit-mask-image:linear-gradient(to right,#000 0,#000 calc(100% - 24px),transparent 100%)}
        .waitlist-view-tabs::-webkit-scrollbar{display:none}
        .waitlist-view-tabs button{display:inline-flex;align-items:center;justify-content:center;gap:8px;flex:0 0 auto;min-width:max-content;min-height:46px;padding:9px 12px;border:1px solid transparent;border-radius:11px;background:transparent;color:#334155;font-size:.84rem;font-weight:700;line-height:1;white-space:nowrap;text-align:left;box-shadow:none;scroll-snap-align:center}
        .waitlist-view-tabs button:hover{background:#f8fafc;color:#334155;box-shadow:none}
        .waitlist-view-tabs button.active{border:1px solid #bfd8ff;background:#eaf2ff;color:#1463df;box-shadow:inset 0 0 0 1px rgba(37,99,235,.06),0 4px 12px rgba(37,99,235,.12)}
        .waitlist-view-tabs button svg{width:18px;height:18px;flex:0 0 auto}
        .waitlist-tab-count{min-width:23px;height:23px;padding:0 7px;font-size:11px}
        .waitlist-toolbar{display:flex;align-items:stretch;flex-direction:column;gap:12px;margin-bottom:14px}
        .waitlist-search{display:flex;width:100%;height:46px;min-height:46px;min-width:0;padding:0 14px;gap:10px;border:1px solid rgba(15,23,42,.10);border-radius:12px;background:#fff;color:#94a3b8;box-shadow:0 1px 2px rgba(15,23,42,.03)}
        .waitlist-search svg{width:18px;height:18px;flex:0 0 auto}
        .waitlist-search input{min-width:0;font-size:14px}
        .waitlist-toolbar__actions{display:grid;width:100%;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:10px;margin-left:0}
        .waitlist-filter-trigger{width:100%;min-width:0;height:46px;padding:0 14px;border:1px solid rgba(15,23,42,.10);border-radius:12px;background:#fff;color:#334155;font-size:14px;box-shadow:0 1px 2px rgba(15,23,42,.03)}
        .waitlist-add-button{align-self:auto;height:46px;min-height:46px;padding:0 14px;border-radius:12px;font-size:14px;white-space:nowrap}
      }
      @media(max-width:540px){
        .waitlist-view-tabs button{min-height:44px;padding:8px 11px;font-size:.8rem}
        .waitlist-view-tabs button svg{width:17px;height:17px}
        .waitlist-search{height:44px;min-height:44px;padding:0 11px;border-radius:14px}
        .waitlist-search svg{width:16px;height:16px}
        .waitlist-search input{font-size:.86rem}
      }
    `}</style>
  </div>
}
