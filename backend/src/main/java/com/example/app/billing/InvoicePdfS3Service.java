...
public class InvoicePdfS3Service {
    ...
    String buildObjectKey(Bill bill) {
        ...
        String tenantCode = safeTenantCode(bill.getCompany());
        ...
        return prefix + "/" + tenantCode + "/invoices/" + year + "/" + billId + "_" + safeNum + ".pdf";
    }

    private static String safeTenantCode(com.example.app.company.Company company) {
        String tenantCode = company == null ? null : company.getTenantCode();
        if (tenantCode == null || tenantCode.isBlank()) {
            return company != null && company.getId() != null ? String.valueOf(company.getId()) : "unknown";
        }
