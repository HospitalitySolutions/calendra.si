...
public class ZoomController {
    ...
    public void callback(
        ...
        if (error != null) {
            response.sendRedirect(frontendBase + "?zoom_error=" + URLEncoder.encode(error, StandardCharsets.UTF_8));
            return;
        }
        if (code == null || state == null) {
            response.sendRedirect(frontendBase + "?zoom_error=missing_code_or_state");
            return;
        }
        try {
            Long userId = Long.parseLong(state);
            zoomService.exchangeCodeForToken(userId, code);
            response.sendRedirect(frontendBase + "?zoom_connected=1");
        } catch (Exception e) {
            String msg = e.getMessage() != null ? e.getMessage() : e.getClass().getSimpleName();
            if (msg.length() > 200) msg = msg.substring(0, 200) + "...";
            response.sendRedirect(frontendBase + "?zoom_error=" + URLEncoder.encode(msg, StandardCharsets.UTF_8));
        }
    }
