...
public class VoiceBookingService {
    ...
    @Transactional
    public VoiceActionResponse handleTranscript(String transcript, User me, boolean confirmCancellation, String localeTag, Long clientIdOverride, String clientNameOverride) {
        ...
        return switch (action) {
            case BOOK_SESSION -> bookFromVoice(me, fields, sessionDate, sessionTime, parsed, zone, lang, confirmCancellation, clientIdOverride, clientNameOverride);
            case CANCEL_SESSION -> {
                Long cancelClientId = clientIdOverride;
                if (cancelClientId == null) {
                    NameParts overrideParts = parseClientName(clientNameOverride);
                    if (!overrideParts.isEmpty()) {
                        cancelClientId = resolveClientId(overrideParts.firstName(), overrideParts.lastName(), me, false, lang);
                    } else {
    ...
    private VoiceActionResponse bookFromVoice(
        ...
        if (clientIdOverride != null) {
            ...
            NameParts overrideParts = parseClientName(clientNameOverride);
            if (!overrideParts.isEmpty()) {
                clientId = resolveClientId(overrideParts.firstName(), overrideParts.lastName(), me, true, lang);
            } else {
                clientId = resolveClientId(fields.firstName(), fields.lastName(), me, true, lang);
            }
        }
    ...
    private NameParts parseClientName(String raw) {
        if (raw == null) {
        ...
        if (lastName.isBlank()) {
            return new NameParts("", firstName.isBlank() ? trimmed : firstName);
        }
