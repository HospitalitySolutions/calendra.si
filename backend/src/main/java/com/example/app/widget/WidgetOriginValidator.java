...
public class WidgetOriginValidator {
    ...
    public void validate(Company company, HttpServletRequest request) {
        ...
        if (allowed.isEmpty()) {
            if (properties.isRequireAllowedOrigin()) {
                throw new ResponseStatusException(HttpStatus.FORBIDDEN, "This tenant has not configured allowed widget websites.");
            }
            return;
        }
    ...
    public Set<String> allowedOrigins(Long companyId) {
        ...
        settings.findByCompanyIdAndKey(companyId, SettingKey.WIDGET_ALLOWED_ORIGINS.name())
                .map(s -> s.getValue())
