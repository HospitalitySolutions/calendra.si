...
public class WidgetTurnstileService {
    ...
    public void verifyForPublicAction(Company company, String token, String remoteIp) {
        if (!isEnabled(company)) {
            if (requiredForPublicActions) {
                throw new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE, "Public widget verification is not configured.");
            }
            return;
        }
        verifyToken(company, token, remoteIp);
    }
    ...
    private void verifyToken(Company company, String token, String remoteIp) {
