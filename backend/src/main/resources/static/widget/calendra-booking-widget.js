(function () {
  ...
  const TRANSLATIONS = {
    ...
    sl: {
      ...
      paymentMethodGiftCard: 'Darilna kartica',
      paymentMethodGiftCardSubtitle: 'Uporabite razpoložljivo dobroimetje darilne kartice.',
      ...
      summaryDeposit: 'Polog',
      summaryDueNow: 'Plačilo danes',
      summaryRemaining: 'Preostanek na lokaciji',
      depositPaymentNote: 'Ob rezervaciji se zaračuna polog; preostanek se poravna na lokaciji.',
    ...
    detailsPaymentSummaryMarkup(service) {
      ...
      if (!this.requiresOnlinePaymentChoice() || requirement === 'none') {
        return `
          ...
          <div class="summary-row">
            <span class="summary-row-label">${escapeHtml(t.summaryPayment)}:</span>
            <strong class="summary-row-value">${escapeHtml(t.summaryPayAtVenue)}</strong>
          </div>
        `;
      }
      if (requirement !== 'deposit') {
        return `
          ...
          <div class="summary-row">
            <span class="summary-row-label">${escapeHtml(t.summaryPayment)}:</span>
            <strong class="summary-row-value">${escapeHtml(t.summaryFullPayment)}</strong>
          </div>
        `;
      }
      ...
      return `
        ...
        <div class="summary-row">
          <span class="summary-row-label">${escapeHtml(t.summaryPayment)}:</span>
          <strong class="summary-row-value">${escapeHtml(t.summaryDeposit)}</strong>
        </div>
        <div class="summary-row">
          <span class="summary-row-label">${escapeHtml(t.summaryDueNow)}:</span>
          <strong class="summary-row-value">${escapeHtml(depositLabel)}</strong>
        </div>
        ${remainingLabel ? `
          <div class="summary-row">
            <span class="summary-row-label">${escapeHtml(t.summaryRemaining)}:</span>
            <strong class="summary-row-value">${escapeHtml(remainingLabel)}</strong>
          </div>
        ` : ''}
                ...
                ` : `
                  ...
                  ${hasAnyPaymentMethod ? `
                    <div class="payment-methods-grid">
                      ${allowed.giftCard ? methodCard('GIFT_CARD', t.paymentMethodGiftCard, t.paymentMethodGiftCardSubtitle, true) : ''}
    ...
    styles() {
      return `
        ...
        * { box-sizing: border-box; }
        ...
        .summary-payment-note { margin: -2px 0 0; color: var(--calendra-muted); font-size: 13px; line-height: 1.45; }
