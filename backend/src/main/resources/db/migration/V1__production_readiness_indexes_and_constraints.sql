-- Production-readiness baseline for existing Calendra PostgreSQL schemas.
-- Flyway is enabled only in staging/production profiles; local dev can still use Hibernate ddl-auto=update.
-- The migration is intentionally defensive: it checks table existence so it can run safely during phased rollout.
